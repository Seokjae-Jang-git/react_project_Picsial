...
router.post('/upload', upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        const { title, description, tags, categoryId, userNo } = req.body;

        connection = await db.getConnection();
        
        // 💡 1. PS_PHOTO 테이블에 INSERT (CATEGORY_ID 제외) 및 생성된 PHOTO_ID 반환받기
        const insertPhotoSql = `
            INSERT INTO PS_PHOTO (PHOTO_ID, USER_NO, TITLE, DESCRIPTION, IMAGE_URL, THUMB_URL)
            VALUES (PS_PHOTO_SEQ.NEXTVAL, :userNo, :title, :description, :imageName, :thumbName)
        ...
        const photoResult = await connection.execute(insertPhotoSql, {
            ...
            // tags: tags || '',
            newPhotoId: { type: oracledb.NUMBER, dir: oracledb.BIND_OUT } // 새로 생성된 PK 받기
        });

        const generatedPhotoId = photoResult.outBinds.newPhotoId[0];

        // 💡 2. 카테고리가 선택되었다면 PS_PHOTO_CATEGORY 테이블에 매핑 데이터 INSERT
        if (categoryId) {
        ...
        await connection.commit(); // 사진과 카테고리를 하나의 트랜잭션으로 커밋
...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        // 💡 3. photo와 comments를 함께 묶어서 프론트엔드로 보냅니다!
        res.json({ 
            success: true, 
            photo: photoData, 
            comments: commentResult.rows // 댓글 배열 탑재 완료
        });
